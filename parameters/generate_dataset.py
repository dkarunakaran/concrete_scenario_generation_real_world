...
save_dataset = '/openx/parameters/dataset/'

class GenerateParameterSpace:
    def __init__(self):
        self.save_loc = save_dataset + 'parameter_space/'
        self.cut_in = {
    ...
    def process(self, _file, filename):
        ...
        for data in all_data['parameter']:
            if _type == 'cutin':
                ...
                self.cut_in['param_relative_lane_init'].append(self.relative_lane(data['param_ego_lane_no_init'],data['param_adv_lane_no_init'], 'cut_in init'))
            elif _type == 'cutout':
                # cut_out 
                ...
                self.cut_out['param_relative_lane_final'].append(self.relative_lane(data['param_ego_lane_no_init'], data['param_adv_lane_no_final'], 'cut_out init'))
...
class GenerateDataset:
    ...
    def generate(self, _file, filename):
        with open(_file) as f:
            all_data = json.loads(f.read())
            _type = all_data['type']
            print("type: {}".format(_type))
            count = 1
            for data in all_data['parameter']:
                name = filename+"_"+_type+"_"+str(count)+".json"          
                json_data = {
                    'type': _type,
                    'data': data
                }
                with open(save_dataset+name, 'w') as outfile:
                    json.dump(json_data, outfile)
                count += 1 
                 

if __name__ == '__main__':
    ps = GenerateParameterSpace()
    ...
    for _file in _dir:
        ...
        if fileDetails[1] == '.cutin' or fileDetails[1] == '.cutout':
            ...
            dataset.generate(loc+_file, fileDetails[0])
            ps.process(loc+_file, fileDetails[0])
            ps.save()    
