...
_file = 'esmini_real_plot_data_cut_out.json'
...
s_cut_diff = 0
if 'adv_real_esmini_s_diff' in adversary_esmini_sec3.keys():
    s_cut_diff = adversary_esmini_sec3['adv_real_esmini_s_diff']
print("s_cut_diff: {}".format(s_cut_diff))
if 'cut_time' not in adversary_esmini_sec3.keys() or adversary_esmini_sec3['cut_time'] is None:
    adversary_esmini_sec3['cut_time'] = adversary_real_sec3['cut_time']
...
adversary_real_sec3['cut_time'] = 6 #5
adversary_esmini_sec3['cut_time'] = 4 #5
...
plt.figure()
plt.figure(figsize=(37,37)) #15 25 #35 60
plt.xlabel("t, lateral displacement", size=110)
plt.ylabel('s, longitudinal displacement',size=110)
plt.xlim([-1.0, -5.0])
plt.ylim([0, 90])
plt.text(-1.25, 22, "rmse_s = {:.3f} m".format(rmse_s), fontsize = 110)
plt.text(-1.25, 16, "rmse_t = {:.3f} m".format(rmse_t), fontsize = 110)
...
for index in range(len(s_real_sec)-2): 
...
plt.tick_params(axis='x', labelsize=110)
plt.tick_params(axis='y', labelsize=110)
plt.legend(['Real-world', 'OpenSCENARIO'], prop={'size':110})
plt.savefig(name)
plt.close()
...
name = path_to_save_dir+"rss_noise2"
key = 'rss'
plt.figure()
...
cut_time_esmini_without_dist = 4

# First noise scneario
adversary_esmini_with_dist = data['data'][1]['adversary_esmini']
adversary_esmini_sec_with_dist = data['data'][1]['adversary_esmini_sec']
cut_time_esmini_with_dist = 4

# Second noise scenario
adversary_esmini_sec_with_dist2 = data['data'][2]['adversary_esmini_sec']
adversary_esmini_with_dist2 = data['data'][2]['adversary_esmini']
...
plt.plot(t_esmini_without_d, s_esmini_without_d, color='g',  linewidth=20)
color = 'm'
plt.plot(t_esmini_with_d, s_esmini_with_d, color='y',  linewidth=20)

plt.plot(t_esmini_with_d2, s_esmini_with_d2, color='r',  linewidth=20)
...
for index in range(len(s_real_sec)): 
    if index >= cut_time_esmini_without_dist-1:
        ...
        plt.plot(adversary_esmini_sec_without_dist['t'][index]-0.05,s_esmini_sec_without_d[index],marker='o',markersize=r_size,color='g')
...
for index in range(len(s_real_sec)): 
    if index >= cut_time_esmini_with_dist-1:
        ...
        plt.plot(adversary_esmini_sec_with_dist['t'][index]+0.05,s_esmini_sec_with_d[index],marker='o',markersize=r_size,color='y')
...
for index in range(len(s_real_sec)): 
    if index >= cut_time_esmini_with_dist2-1:
        ...
        plt.plot(adversary_esmini_sec_with_dist2['t'][index],s_esmini_sec_with_d2[index],marker='o',markersize=r_size,color='r')
...
plt.legend(['Without disturbance', 'Added 3', 'Reduced 3'], prop={'size':100})
plt.savefig(name)
plt.close()
